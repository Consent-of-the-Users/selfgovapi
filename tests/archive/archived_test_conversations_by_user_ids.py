...
def test_get_convo_by_user_list(method, make_request, is_authorized, is_valid_post_data):
    """
    Test the GET /convos/ endpoint.
    Should receive a list of user.uids and return a convo with those and only those participants.
    """
    method = normalized_put_method_name(method)

    from conftest import mock_with_patch
    mock_class = mock_with_patch("routes.convo.convo.are_valid_participants")
    ...
    response = make_request(method, "/v1/convos/?users=1,12,123", authorization=is_authorized)
